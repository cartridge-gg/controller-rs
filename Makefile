...
# Build files helpers.
build := ./target/dev/controller_account_
...
generate_artifacts:
	scarb build
	mkdir -p ${store}
	jq . ${build}ControllerAccount${sierra} > ${store}controller.latest.contract_class.json
	cp ${build}ControllerAccount${compiled} ${store}controller.latest.compiled_contract_class.json
