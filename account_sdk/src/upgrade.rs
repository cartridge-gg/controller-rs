...
pub fn determine_upgrade_path(
    current_version: Option<&ControllerVersionInfo>,
    is_beta: bool,
) -> Result<UpgradePath, ControllerError> {
    let metadata = ControllerMetadata::load()?;
    let target_version_key = if is_beta {
        &metadata.beta_controller
    } else {
        &metadata.stable_controller
    };
    ...
    let available = match current_version {
        Some(current) => {
            let current_index = get_version_index(&current.version);
            let target_index = get_version_index(&target_version_info.version);
...
pub fn find_version_by_hash(hash: Felt) -> Result<Option<ControllerVersionInfo>, ControllerError> {
    let metadata = ControllerMetadata::load()?;
    ...
    for (version_key, controller_info) in metadata.controllers.iter() {
        if controller_info.class_hash == hash_str && version_key != "latest" {
            return Ok(controller_info.version_info.clone());
        }
    }
...
fn get_version_index(version: &str) -> Option<usize> {
    ...
    ordered_versions.iter().position(|&v| v == version)
}
...
impl Controller {
    ...
    pub async fn current_version(&self) -> Result<Option<ControllerVersionInfo>, ControllerError> {
        let class_hash = self
            .provider
    ...
    pub async fn check_upgrade_path(&self, is_beta: bool) -> Result<UpgradePath, ControllerError> {
        let current_version = self.current_version().await?;
        determine_upgrade_path(current_version.as_ref(), is_beta)
    }
    ...
    pub async fn execute_upgrade(
        ...
        match current_version {
            Some(version) => match version.outside_execution_version {
